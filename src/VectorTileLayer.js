import { Util, Support } from "esri-leaflet";
import {
  loadStyle,
  formatStyle,
  getAttributionData,
...
export var VectorTileLayer = L.Layer.extend({
  statics: {
  ...
  initialize: function(key, options) {
    ...
    if (
      ...
      L.setOptions(this, VectorTileLayer.STYLES[options.key].options);
      L.setOptions(this, {
        styleId: VectorTileLayer.STYLES[options.key].id
      });
      L.setOptions(this, options);
    } else if (typeof options.key === "string") {
      L.setOptions(this, { styleId: options.key });
      L.setOptions(this, options);
    }
    ...
    loadStyle(
      ...
      function(error, style, service, serviceUrl) {
        this._setupStyle.call(this, style, service, serviceUrl);
      }.bind(this)
    );
  },

  _setupStyle(style, service, serviceUrl) {
  ...
  _setupAttribution() {
    if (this._map && this.options.attributionUrls) {
      ...
      updateMapAttribution({ target: this._map });
    }
  },
  ...
  onRemove: function(map) {
    map.off("moveend", updateMapAttribution);
  ...
  _asyncAdd: function() {
    ...
    if (this._mapboxGL.options.style.layers[0].type === "background") {
      map.getContainer().style.background = this._mapboxGL.options.style.layers[0].paint[
        "background-color"
      ];
    }

    map.on("moveend", updateMapAttribution);
